...
if ~exist('frame')
    frame(:,:,1)=(I+50)*1.5; %wut, what is this for
    frame(:,:,2)=(I+50)*1.5;
    frame(:,:,3)=(I+50)*1.5;
end
MarkerAreaThresh=0; %no markers on gel
SE=ones(7);
RadiusRange=[40 60]; % the preset range of the circle radius
RadiusTestWidth=50;
...
%interactive menu to calibrate ball---I think we should replace with edge
%detecting to find any shape and filter eveything outside of it
